...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			/* 渠道下拉 */
			".dup-select{height:32px;padding:0 8px;border:1px solid var(--dsw-alias-border-l2);border-radius:8px;background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-primary);font-size:13px;font:inherit;cursor:pointer}",
			".dup-select:focus{outline:none;border-color:var(--dsw-alias-brand-primary)}",
			/* changelog 弹窗 */
		...
		function fetchChangelog() {
			return fetch(CHANGELOG_URL, { method: "GET", cache: "no-store" }).then((res) => {
				if (!res.ok) {
					throw new Error("更新内容服务不可用（HTTP " + res.status + "），请重启 harness 后重试");
				}
									...
									jsx("span", {
										...
										children: [
											jsx("select", {
												...
												children: CHANNELS.map((ch) => jsx("option", {
													key: ch,
													value: ch,
											...
											jsx("span", {
												className: "dup-note",
												children: "检查更新以选中渠道的最新版本为目标（rc = 发布候选，stable = 正式版，alpha = 预发布）"
											})
										]
									})
								]
		...
		/** GitHub 官方 octocat 标志（16×16，跟随 currentColor）。 */
		function GitHubLogo() {
			return jsx("svg", {
				width: 16,
				height: 16,
