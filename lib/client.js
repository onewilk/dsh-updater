...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const { BrandWordmark, MarkdownText } = require("@deepseek-ai/dsh-client-ui-primitives");
		...
		const css = [
			...
			".dup-md{font-size:13px;line-height:20px;color:var(--dsw-alias-label-primary)}",
			".dup-note{font-size:12px;line-height:18px;color:var(--dsw-alias-label-tertiary)}",
			/* 渠道选择 */
			".dup-channel{display:flex;gap:6px;align-items:center;flex-wrap:wrap}",
			".dup-channel-btn{height:28px;padding:0 12px;border-radius:14px;border:1px solid var(--dsw-alias-border-l2);background:transparent;color:var(--dsw-alias-label-secondary);font-size:12px;cursor:pointer;font:inherit;line-height:1}",
			".dup-channel-btn:hover{background:var(--dsw-alias-interactive-bg-hover)}",
			".dup-channel-btn[aria-pressed=true]{background:var(--dsw-alias-button-primary-fill);color:var(--dsw-alias-label-primary-foreground);border-color:transparent}",
			/* changelog 弹窗 */
			".dup-modal-overlay{position:fixed;inset:0;z-index:1100;background:rgba(0,0,0,.45);display:flex;align-items:center;justify-content:center;padding:24px}",
			".dup-modal{width:min(640px,100%);max-height:80vh;display:flex;flex-direction:column;background:var(--dsw-alias-bg-module-platform);border:1px solid var(--dsw-alias-border-l2);border-radius:14px;box-shadow:0 24px 64px rgba(0,0,0,.3);overflow:hidden}",
			".dup-modal-head{flex:none;display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid var(--dsw-alias-border-l2)}",
			".dup-modal-title{font-size:15px;font-weight:600;color:var(--dsw-alias-label-primary)}",
			".dup-modal-close{width:28px;height:28px;border:none;background:transparent;color:var(--dsw-alias-label-secondary);cursor:pointer;border-radius:8px;font-size:16px;line-height:1;display:flex;align-items:center;justify-content:center}",
			".dup-modal-close:hover{background:var(--dsw-alias-interactive-bg-hover)}",
			".dup-modal-body{flex:1;min-height:0;overflow-y:auto;padding:16px 18px;font-size:13px;line-height:20px;color:var(--dsw-alias-label-primary)}",
			/* 开关（使用 GitHub 加速） */
		...
		const UPGRADE_URL = "/plugins/dsh-updater/upgrade";
		const CHANGELOG_URL = "/plugins/dsh-updater/changelog";
		/** 可用更新渠道（与服务端一致）。 */
		const CHANNELS = ["rc", "stable", "alpha"];
		const DEFAULT_CHANNEL = "rc";
		const CHANNEL_LABELS = { rc: "rc", stable: "stable", alpha: "alpha" };
		...
		/** 更新渠道的 localStorage key。 */
		const CHANNEL_KEY = "dsh-updater:channel";
		...
		function readChannelPref() {
			try {
				const v = window.localStorage.getItem(CHANNEL_KEY);
				return v && CHANNELS.includes(v) ? v : DEFAULT_CHANNEL;
			} catch {
				return DEFAULT_CHANNEL;
			}
		}
		function writeChannelPref(value) {
			try {
				window.localStorage.setItem(CHANNEL_KEY, String(value));
			} catch {}
		}
		...
		function fetchStatus(force, proxy, channel) {
			if (force) {
				return fetch(CHECK_URL, {
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({ proxy: !!proxy, channel }),
					cache: "no-store"
		...
		function saveConfig(proxy, channel) {
		...
		/** 按需获取最新版本的更新内容（markdown）。 */
		function fetchChangelog() {
			return fetch(CHANGELOG_URL, { method: "GET", cache: "no-store" }).then((res) => parseJson(res));
		}
		...
		/** 「关于」Tab 的版本信息卡片。 */
		function StatusBody({ data, checking, onCheck, onViewChangelog }) {
			...
			/** 升级命令：用实际最新版本号，而非 @latest（因为新版可能在 next 等 tag 下）。 */
			const upgradeCmd = latest ? "npx @deepseek-ai/dsh@" + latest + " web" : "npx @deepseek-ai/dsh@latest web";
			...
			const onUpgrade = () => {
				setUpgrading(true);
				fetch(UPGRADE_URL, { method: "POST" })
					.then((r) => {
						if (!r.ok) { setUpgrading(false); return null; }
						return r.json();
					})
					.then((j) => {
						if (j && !j.ok) setUpgrading(false);
						// j.ok === true 时进程即将退出，保持 upgrading 状态
					})
					.catch(() => {
						// 网络错误（进程退出导致连接断开是正常的，但给个兜底恢复）
						setTimeout(() => setUpgrading(false), 10000);
					});
			};
				...
				children: [
								...
								children: [
									...
									jsx("span", {
										...
										children: [
											...
											statusTag,
											updateAvailable && jsx("button", {
												type: "button",
												className: "dup-btn dup-btn-ghost",
												style: { height: 28, padding: "0 10px", borderRadius: 14, fontSize: 12 },
					...
					updateAvailable && jsxs("div", {
						...
						children: [
							...
							jsx("p", { className: "dup-note", children: upgrading ? "正在停止当前进程并重启…页面即将断开，请稍后刷新。" : "下方命令可手动执行，也可点击按钮自动重启：" }),
							jsxs("div", {
								className: "dup-code-row",
								children: [
									jsx("code", { className: "dup-mono", children: upgradeCmd }),
									jsx("button", {
										type: "button",
										className: "dup-btn dup-btn-ghost",
										onClick: onCopy,
							...
							jsx("button", {
								...
								disabled: upgrading,
								style: { marginTop: 12 },
								children: upgrading ? "正在重启…" : "一键更新并重启"
							})
						]
					})
				]
		...
		function AboutSection() {
			...
			const [channel, setChannel] = React.useState(readChannelPref);
			/** changelog 弹窗状态：null 关闭；{loading} / {data} / {error}。 */
			...
			const applyResult = React.useCallback((j) => {
				...
				if (j && CHANNELS.includes(j.channel)) setChannel(j.channel);
			}, []);

			const load = React.useCallback((force, ch, pr) => {
				fetchStatus(force, pr, ch)
					.then(applyResult)
					.catch((err) => setData({ error: String(err && err.message ? err.message : err) }));
			}, [applyResult]);

			React.useEffect(() => {
				load(false, channel, proxy);
			}, []);

			const onCheck = () => {
				setChecking(true);
				fetchStatus(true, proxy, channel)
					.then(applyResult)
					.catch((err) => setData({ error: String(err && err.message ? err.message : err) }))
			...
			const changeChannel = (ch) => {
				...
				writeChannelPref(ch);
				setChecking(true);
				saveConfig(proxy, ch).catch(() => {});
				fetchStatus(true, proxy, ch)
					.then(applyResult)
					.catch(() => {})
					.finally(() => setChecking(false));
			};
			...
			const openChangelog = () => {
				setChangelog({ loading: true });
				fetchChangelog()
					.then((j) => setChangelog({ data: j }))
					.catch((err) => setChangelog({ error: String(err && err.message ? err.message : err) }));
			};
			const closeChangelog = () => setChangelog(null);
			...
			return jsxs("div", {
				...
				children: [
					...
					jsx(StatusBody, { data, checking, onCheck, onViewChangelog: openChangelog }),
					jsxs("div", {
						className: "dup-card",
						children: [
							jsx("span", { className: "dup-rowLabel", children: "更新渠道" }),
							jsx("div", {
								className: "dup-channel",
								children: CHANNELS.map((ch) => jsx("button", {
									key: ch,
									type: "button",
									className: "dup-channel-btn",
									"aria-pressed": ch === channel,
									onClick: () => changeChannel(ch),
									children: CHANNEL_LABELS[ch] || ch
								}))
							}),
							jsx("p", {
								className: "dup-note",
								children: "检查更新以选中渠道的最新版本为目标。rc = 发布候选（默认）；stable = 正式版；alpha = alpha 预发布。"
							})
						]
					}),
					jsxs("div", {
					...
					changelog && jsx(ChangelogModal, { state: changelog, onClose: closeChangelog })
				]
			});
		}

		/** 更新内容弹窗：点击「查看更新内容」后弹出，用 MarkdownText 渲染 markdown。 */
		function ChangelogModal({ state, onClose }) {
			if (!state) return null;
			const body = state.data && typeof state.data.body === "string" ? state.data.body : "";
			return jsx("div", {
				className: "dup-modal-overlay",
				onClick: onClose,
				children: jsx("div", {
					className: "dup-modal",
					...
					children: [
						jsxs("div", {
							className: "dup-modal-head",
							children: [
								jsx("span", { className: "dup-modal-title", children: "更新内容" }),
								jsx("button", {
									type: "button",
						...
						jsx("div", {
							className: "dup-modal-body",
							children: state.loading
								? jsx("span", { className: "dup-note", children: "加载中…" })
								: state.error
									? jsx("span", { className: "dup-note", children: "获取失败：" + state.error })
									: body.length > 0
										? jsx("div", { className: "dup-md", children: jsx(MarkdownText, { text: body }) })
										: jsx("span", { className: "dup-note", children: "暂无更新内容" })
						})
					]
				})
			});
