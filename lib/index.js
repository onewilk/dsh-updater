...
/**
 * 从 npm registry 读取最新版本（dist-tags.latest）。
 * @returns 版本字符串；失败抛异常。
 */
async function fetchNpmLatest() {
	...
	const latest = json && json["dist-tags"] && typeof json["dist-tags"].latest === "string" ? json["dist-tags"].latest : null;
	if (!latest) throw new Error("npm registry 缺少 dist-tags.latest");
	return latest;
}
